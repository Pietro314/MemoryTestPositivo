...
# Awk-free pra rodar em Android 7 (toybox antigo sem awk no device).
# Single-quote no comando do adb shell pra nao precisar escapar $.
MEM_TOTAL_KB=$(adb shell 'grep MemTotal /proc/meminfo 2>/dev/null | { read _ v _; echo "$v"; }' 2>/dev/null | tr -d '\r')
MEM_AVAIL_KB=$(adb shell 'grep MemAvailable /proc/meminfo 2>/dev/null | { read _ v _; echo "$v"; }' 2>/dev/null | tr -d '\r')
...
# espaco livre /data — awk-free: pula header, le 4a coluna do data row
STOR_FREE_KB=$(adb shell 'df /data 2>/dev/null | { read; read _ _ _ v _; echo "$v"; }' 2>/dev/null | tr -d '\r')
