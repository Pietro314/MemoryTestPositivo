...
if [ "$TYPE" = "UNKNOWN" ]; then
    ...
    if [ -n "$CID_PATH" ] && [ -f "$CID_PATH" ]; then
        ...
        if [ "$LIFE_A" = "N/A" ] || [ "$LIFE_B" = "N/A" ]; then
            ...
            if [ -n "$LIFE_RAW" ]; then
                # Awk-free: usa set -- pra dividir LIFE_RAW por whitespace
                LIFE_A_TMP=$(set -- $LIFE_RAW; echo "$1")
                LIFE_B_TMP=$(set -- $LIFE_RAW; echo "$2")
...
if [ "$STOR_GB" = "N/A" ] || [ "$STOR_GB" = "0" ]; then
    ...
    # Awk-free: pula header (1o read) e le 2a coluna do data row (2o read).
    DF_KB=$(df /data 2>/dev/null | { read; read _ v _; echo "$v"; })
    log_debug "df /data total KB='${DF_KB:-<vazio>}'"
    if [ -n "$DF_KB" ] && [ "$DF_KB" -gt 0 ] 2>/dev/null; then
        # Awk-free + sem overflow no mksh 32-bit (DF_KB pode passar de 2^31
        # em chips > 2TB). Mesma tecnica do STOR_BYTES.
        dlen=${#DF_KB}
        if [ "$dlen" -gt 4 ]; then
            mb_approx=${DF_KB%???}
            STOR_GB=$(( mb_approx / 1024 ))
        else
